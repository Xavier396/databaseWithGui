...
public class DeliverManager {
    ...
    private int addWorker() {
        ...
        ResultSet rs1=BasicDao.executeQuery("select count(*) from express_workers");
        try {
            if (rs1.next())
            {
                workerId+=rs1.getInt(1);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        //拼接字符串
        BasicDao.executeUpdate("insert into express_workers(worker_id,worker_name,worker_gender,worker_role,worker_phone,worker_birthday,worker_pwd,worker_work_time,worker_finish_time) values('"+workerId+"','"+worker_name+"','"+worker_gender+"','"+"D"+"','"+worker_phone+"','"+worker_birthday.toString()+"','"+worker_password+"','"+worker_start_time+"','"+worker_finish_time+"')");
        Utils.dialogCreator(0,"成功","帐号:"+workerId+"\n"+"密码:"+worker_password,"请妥善保存,切勿随意泄露");
        name.setText("");
        phone.setText("");
    ...
    private void deleteWorker() {
        if (isQueried) {
            ...
            Utils.dialogCreator(3, "错误", "在执行操作前,请先查询信息", "为查询信息");
        }
    }
    @FXML
    private int query() {
